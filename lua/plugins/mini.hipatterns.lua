return {
    ...
    opts = function()
        ...
        local get_fg = function(hex)
            ...
            return (0.299 * r + 0.587 * g + 0.114 * b) > 128 and '#1e1e1e'
                or '#dbdbdb'
        end
        ...
        local compute_rgb = function(_, match)
            local r, g, b =
                match:match 'rgba?%(%s*(%d+)%s*,%s*(%d+)%s*,%s*(%d+)'
        ...
        local compute_hsl = function(_, match)
            local h, s, l =
                match:match 'hsla?%(%s*(%d+)%s*,%s*(%d+)%%%s*,%s*(%d+)%%'
    ...
    config = function(_, opts)
        ...
        vim.api.nvim_set_hl(
            0,
            'MiniHipatternsFix',
            { fg = '#1E1E1E', bg = '#F34E4E', bold = true }
        ) -- Red
        vim.api.nvim_set_hl(
            0,
            'MiniHipatternsHack',
            { fg = '#1E1E1E', bg = '#cfcf2a', bold = true }
        ) -- Yellow
        vim.api.nvim_set_hl(
            0,
            'MiniHipatternsTodo',
            { fg = '#1E1E1E', bg = '#38d5d5', bold = true }
        ) -- Cyan
